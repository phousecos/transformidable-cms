...
export default function HeroBookClub({ book, dateLabel }: HeroBookClubProps) {
  ...
  return (
    <>
              ...
              <div className="mt-4 shrink-0">
                {book.bookshop_url && (
                  <a href={book.bookshop_url} target="_blank" rel="noopener noreferrer"
                    className="inline-block rounded-sm border border-parchment/60 px-6 py-2.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-parchment transition-colors hover:bg-parchment/10 focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-gold md:text-xs">
      ...
      {/* Dialog: focus is trapped, Escape closes, focus returns to trigger. */}
      {showModal && (
        <div
        ...
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="hero-bookclub-dialog-title"
            tabIndex={-1}
          ...
          >
            ...
            <div className="flex items-start gap-5">
              {book.coverUrl && (
                <img src={book.coverUrl} alt="" className="w-24 shrink-0 shadow-md" />
              )}
              <div>
                <h3 id="hero-bookclub-dialog-title" className="font-serif text-xl font-bold text-obsidian">{book.title}</h3>
                <p className="mt-1 text-sm text-obsidian/70">{book.author}</p>
              </div>
            </div>
